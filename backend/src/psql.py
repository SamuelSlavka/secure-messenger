...
def get_messages(raddress, saddress):
   """ return all mesages between two addreses """
   conn = connect_db()
   cur = conn.cursor()
   cur.execute("SELECT * FROM message WHERE (recvAddress=%s AND sendAddress=%s) OR (recvAddress=%s AND sendAddress=%s);",
               (raddress, saddress, saddress, raddress))
   res = []
   for row in cur:
      res.append(row)
   cur.close()
...
def set_message(recv_address, send_address, recv_name, send_name, timestamp, recv_contents, send_contents):
   ...
   cur.execute("INSERT INTO message (recvAddress, sendAddress, recvName, sendName, timestamp, recvContents, sendContents) VALUES (%s, %s, %s, %s, %s, %s, %s)",
